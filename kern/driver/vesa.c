...
void vesa_get_mode(uint16_t mode, mode_info_t * mode_info) {
    ...
    memcpy(mode_info, (void*)0xd000, sizeof(mode_info_t));
}
...
uint32_t vesa_find_mode(uint32_t width, uint32_t height, uint32_t bpp) {
    ...
    reg_in.di = 0xd500;
    bios32_service(BIOS_GRAPHICS_SERVICE, &reg_in, &reg_out);
    memcpy(&vbe_info, (void*)0xd500, sizeof(vbe_info_t));
...
void vesa_init() {
    ...
    cprintf("vesa set mode success\n");
    //qemu_printf("Will there be any more irqs? eflags = %08x\n", eflags);
    //write_serial('w');
    //asm volatile("int $0x20");
    //irq_ack(0x28);
    void * t = vesa_get_lfb();
    cprintf("display addr: %p, %p\n", t, t + 1024 * 768 * 4);
    assert((uint32_t)t >= 0xFB000000);
    //allocate_region(kpage_dir, (uint32_t)t, (uint32_t)(t + 1024*768*4), 1,1,1);
    cprintf("vesa init success\n");
    boot_map_segment(boot_pgdir, (uintptr_t)t, 1024 * 768 * 4, (uintptr_t)t, PTE_W);
}
